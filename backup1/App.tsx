...
const EditableText: React.FC<{
  ...
  className?: string;
  multiline?: boolean;
}> = ({ value, onSave, isEditing, className = "", multiline = false }) => {
  ...
  return (
    <span className={`relative group/edit inline-block w-full ${className}`}>
      {multiline ? (
        <textarea
          value={localValue}
          ...
          className="w-full bg-indigo-500/10 border border-indigo-500/30 rounded-sm p-2 text-inherit focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all resize-none"
          rows={4}
        />
      ) : (
        <input
          ...
          onFocus={() => setIsFocused(true)}
          className="w-full bg-indigo-500/10 border border-indigo-500/30 rounded-sm px-2 py-1 text-inherit focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all"
        />
      )}
      {!isFocused && (
        <span className="absolute -top-6 right-0 opacity-0 group-hover/edit:opacity-100 transition-opacity pointer-events-none">
          <span className="text-[8px] font-black uppercase tracking-widest bg-indigo-600 text-white px-2 py-1 rounded-full shadow-lg">Click to Edit</span>
        </span>
      )}
    </span>
  );
...
const Navbar: React.FC<{ 
  ...
  isEditing: boolean;
  onEditToggle: () => void;
}> = ({ siteName, onAdminToggle, isEditing, onEditToggle }) => {
      ...
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all ${isEditing ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-white/5 border-white/10 text-gray-400 hover:text-white'}`}
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">{isEditing ? 'Live Edit ON' : 'Live Edit'}</span>
          </button>
    ...
    <div className="grid lg:grid-cols-[1fr_2.5fr] gap-20 items-start">
          ...
          <div className="relative aspect-[3/4] overflow-hidden rounded-2xl border border-white/10 shadow-2xl bg-zinc-900">
            ...
            {isEditing && (
              <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <p className="text-[10px] font-black uppercase tracking-widest text-white">Use Admin Dashboard to change image</p>
              </div>
            )}
          </div>
        </div>
        <h1 className="text-3xl font-brand font-black text-white mb-2 leading-tight">
          <EditableText value={profile?.name} onSave={(val) => onUpdate({ name: val })} isEditing={isEditing} />
        </h1>
        <div className="text-indigo-400 font-bold uppercase tracking-widest text-[10px] mb-8">
          <EditableText value={profile?.title} onSave={(val) => onUpdate({ title: val })} isEditing={isEditing} />
        </div>
      ...
      <div className="space-y-16">
        <div>
          ...
          <div className="text-2xl md:text-4xl text-white font-medium leading-[1.3] mb-12">
            <EditableText value={profile?.bio} onSave={(val) => onUpdate({ bio: val })} isEditing={isEditing} multiline />
          </div>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <Card key={i} className="p-10 border-white/5 hover:border-indigo-500/20 transition-all group bg-zinc-900/40">
              <h3 className="text-indigo-500 font-black uppercase tracking-widest text-[10px] mb-6 flex items-center gap-3">
                ...
                <EditableText 
                  value={section.title} 
                  onSave={(val) => {
                    const newSections = [...profile.detailedSections];
                    newSections[i] = { ...newSections[i], title: val };
                    onUpdate({ detailedSections: newSections });
                  }} 
                  isEditing={isEditing} 
                />
              </h3>
      ...
      <div className="max-w-7xl mx-auto px-6 relative animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <h1 className="text-5xl md:text-8xl font-brand font-black text-white mb-8 leading-[1.1] tracking-tight max-w-5xl">
          <EditableText 
            ...
            onSave={(val) => onUpdate({ heroHeading: val })} 
            isEditing={isEditing} 
          />
        </h1>
        <div className="text-xl md:text-2xl text-gray-400 mb-12 max-w-3xl leading-relaxed">
          <EditableText 
            value={config?.heroSubheading} 
            onSave={(val) => onUpdate({ heroSubheading: val })} 
            isEditing={isEditing} 
            multiline 
          />
        </div>
...
export default function App() {
  ...
  return (
    <Router>
      <ScrollToTop />
      <div 
        className="min-h-screen bg-[#020205] selection:bg-indigo-500/30 text-white relative"
        style={{ fontFamily: state.config.fontFamily }}
      >
        <BackgroundLayer />
        <Navbar 
        ...
        <Routes>
          <Route path="/" element={<main><Hero config={state.config} isEditing={isEditing} onUpdate={updateConfig} /></main>} />
          <Route path="/profile" element={<ProfileView profile={state.profile} isEditing={isEditing} onUpdate={updateProfile} />} />
