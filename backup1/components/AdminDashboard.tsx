        ...
        {activeTab === 'profile' ? (
          ...
          <Card className="p-8 space-y-6 animate-in fade-in duration-500">
            ...
            <div className="w-full">
              <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Typography</label>
              <select 
                value={state.config.fontFamily}
                onChange={(e) => onUpdateConfig({...state.config, fontFamily: e.target.value})}
                className="w-full bg-black border border-white/10 rounded-sm px-4 py-3 text-white focus:outline-none focus:border-indigo-500 transition-all text-sm appearance-none"
              >
                {FONTS.map(f => (
